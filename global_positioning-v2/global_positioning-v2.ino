...
#define SENSOR_WEIGHT_BACK_ENCODER_L 0.01
#define SENSOR_WEIGHT_BACK_ENCODER_R 0.01
#define SENSOR_WEIGHT_FRONT_ENCODER 0.98
...
void Robot::setupNavX(void){
  ...
  SPI.setClockDivider(32); /* 16Mhz/32 = 500kHz; /16=1Mhz; /8=2Mhz */ 
}
...
int trapVelProfile::getPWM(Robot *robot){
  ...
  if(meters <= this->_x1){
    ...
    if(robot->getRobotVel() <= 0.08){//meters/sec
      return 25;
    }else{
      return 0;  
    }//else keep coasting
...
#define END_MAX_PWM 7.0
#define BEGIN_COASTING 7.3
...
void setup() {
  ...
  velProfile = new trapVelProfile(BEGIN_MAX_PWM, END_MAX_PWM, BEGIN_COASTING, TARGET_POINT, 50, 50);
...
void loop() {
  ...
  */
  ...
  motor->mSpeed(50);//TODO:: Find out why this is causing a CRC Error
