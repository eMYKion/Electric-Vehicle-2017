...
void loop(){
  ...
  if(currSeg <= SEGMENTS && !first_finished_flag){
    ...
    //TODO:: INVESTIGATE
    if(currSeg == SEGMENTS && spline->y(currSeg, 1.0) - y < vehicle->getEstimatedCoastingDistance(vehicle->getRobotVel())){//if on last segment and almost reached goal
      motor->mBreak();
    }else if(y < 8.5){
